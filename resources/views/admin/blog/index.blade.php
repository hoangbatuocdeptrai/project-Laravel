        ...
        <div class="col-md-4 ml-3 mt-4 mb-3">
            <form action="" method="GET" action="" role="form">
                <div class="input-group">
                    ...
                    <a href="{{route('admin.blog.create')}}" class="btn btn-success ml-3"><i class="fa fa-plus"></i>
                        ADD</a>
                </div>
            </form>
        </div>
        ...
        <table class="table table table-sm table-bordered table-hover text-center" style="background: #fff;">
            ...
            <tbody>
                ...
                <tr>
                    <td>{{$bl->id}}</td>
                    <td>{{$bl->name}}</td>
                    <td><img src="{{url('public/upload')}}/{{$bl->image}}" alt="" width="70" height="70"></td>
                    <td>@if($bl->status==1)
                    ...
                    <td class="text-right">
                        <form class="form-inline" action="{{route('admin.blog.destroy',$bl->id)}}" method="POST">
                        @csrf
                            <a href="{{route('admin.blog.edit', $bl->id)}}" class="btn btn-sm btn-primary"><i
